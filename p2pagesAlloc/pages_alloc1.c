...
int init_module(void)
{
  ...
  pgdescriptor = alloc_pages(GFP_KERNEL, ORDER);
  if(!pgdescriptor)
  {
    pr_err("Pages Allocation Failed\n");
    return -ENOMEM;
  }
  ...
  __free_pages(pgdescriptor, ORDER);
...
MODULE_DESCRIPTION("Page Allocator - Interface Functions Set 1");
MODULE_LICENSE("GPL");
